...
const VISIBLE_CATEGORIES = DOCUMENT_TEMPLATE_CATEGORIES.filter(
  (category) => category !== 'attendance',
);

const VISIBLE_SCOPES = DOCUMENT_TEMPLATE_SCOPES.filter(
  (scope) => scope !== 'academic_offering',
);

const CATEGORY_LABELS: Partial<Record<DocumentTemplateCategory, string>> = {
...
const SCOPE_LABELS: Partial<Record<DocumentTemplateScope, string>> = {
...
function getScopeLabel(scope: DocumentTemplateScope): string {
...
function buildInitialValues(inputs: DocumentTemplateInputDefinition[]): Record<string, string> {
  return Object.fromEntries(inputs.map((input) => [input.key, '']));
}

function buildContext(
  values: Record<string, string>,
  optionsByKey: Record<string, DocumentTemplateContextOption[]>,
  ...
  Object.entries(values).forEach(([key, value]) => {
    ...
    const option = optionsByKey[key]?.find((item) => item.value === cleanValue);

    if (option) {
      context[`${key}Label`] = option.description
...
export function DocumentTemplatesPage() {
  const [filters, setFilters] = useState<DocumentTemplateListQuery>(DEFAULT_FILTERS);
  const [templates, setTemplates] = useState<DocumentTemplateDefinition[]>([]);
  const [selectedCode, setSelectedCode] = useState<string | null>(null);
  const [selectedTemplate, setSelectedTemplate] = useState<DocumentTemplateDefinition | null>(null);
  const [variablesData, setVariablesData] = useState<DocumentTemplateVariablesResponse | null>(null);
  const [requiredInputs, setRequiredInputs] = useState<DocumentTemplateInputDefinition[]>([]);
  const [contextOptions, setContextOptions] = useState<Record<string, DocumentTemplateContextOption[]>>({});
  const [inputValues, setInputValues] = useState<Record<string, string>>({});
  const [outputFormat, setOutputFormat] = useState<DocumentOutputFormat>('docx');
  const [validationResult, setValidationResult] = useState<DocumentTemplateValidationResult | null>(null);
  const [generationResult, setGenerationResult] = useState<DocumentTemplateGenerationResult | null>(null);
  ...
  async function loadTemplateDetail(code: string): Promise<void> {
    ...
    try {
      const [template, variables, inputs, options] = await Promise.all([
      ...
      setInputValues(buildInitialValues(inputs.requiredInputs));
      setOutputFormat(template.outputFormats[0] ?? 'docx');
  ...
  async function loadTemplates(nextFilters: DocumentTemplateListQuery = filters): Promise<void> {
    setIsLoadingList(true);
    setError(null);

    try {
      const response = await getDocumentTemplates(nextFilters);
      ...
      const codeToLoad = selectedCode
        && response.items.some((template) => template.code === selectedCode)
        ? selectedCode
        : response.items[0].code;

      await loadTemplateDetail(codeToLoad);
    } catch (loadError: unknown) {
      setError(getErrorMessage(loadError));
    } finally {
  ...
  function setInputValue(key: string, value: string): void {
  ...
  function renderContextField(input: DocumentTemplateInputDefinition) {
    if (isDateInput(input.key)) {
      return (
        <label key={input.key}>
          {input.label}
          <input
            ...
            onChange={(event) => setInputValue(input.key, event.target.value)}
          />
        </label>
      );
    }
    ...
    const selectedOption = options.find((option) => option.value === inputValues[input.key]);
    ...
    return (
      <label key={input.key}>
        ...
        >
          <option value="">
            {options.length === 0 ? 'No hay opciones disponibles' : 'Selecciona una opción'}
          </option>
          {options.map((option) => (
  ...
  async function handleValidationSubmit(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    try {
      const result = await validateDocumentTemplateContext(selectedTemplate.code, {
        outputFormat,
  ...
  async function handleGenerationSubmit(event: FormEvent<HTMLFormElement>): Promise<void> {
    event.preventDefault();
    ...
    try {
      const result = await generateDocumentFromTemplate(selectedTemplate.code, {
        outputFormat,
        context: buildContext(inputValues, contextOptions),
      });
    ...
    <main className="dashboard-content document-templates-page">
      <section className="document-templates-hero">
        <div>
          ...
          <p>
            Selecciona plantillas, elige sus datos desde desplegables y genera documentos registrados en el historial.
          </p>
        </div>
        <span className="document-templates-status">{templates.length} plantillas</span>
      </section>
      ...
      <section className="document-templates-card">
        ...
        <form className="document-templates-filters" onSubmit={(event) => {
          event.preventDefault();
          ...
          <label>
            ...
            <input
              ...
              placeholder="Ej. memoria, evaluación, alumno..."
          ...
          <div className="document-templates-filter-actions">
            <button className="button button-primary" type="submit" disabled={isLoadingList}>
          ...
          <div className="document-templates-list">
            {templates.map((template) => (
              <button
                ...
                key={template.code}
                type="button"
                onClick={() => void loadTemplateDetail(template.code)}
              >
        ...
        <article className="document-templates-card document-templates-detail-card">
          {isLoadingDetail ? (
            ...
            <>
              <div className="document-templates-card-header">
                <div>
                  <p className="eyebrow">{getCategoryLabel(selectedTemplate.category)}</p>
              ...
              <section className="document-template-validation">
                ...
                <p>Elige los datos desde desplegables. No se escribe ningún identificador.</p>

                <form onSubmit={(event) => void handleValidationSubmit(event)}>
                  <label>
                    Formato de salida
                    <select
                      value={outputFormat}
                      onChange={(event) => {
                        setOutputFormat(event.target.value as DocumentOutputFormat);
                        resetResult();
                      }}
                    >
                      {selectedTemplate.outputFormats.map((format) => (
                  ...
                  {requiredInputs.map(renderContextField)}
                ...
                {validationResult ? (
                  ...
                  >
                    <strong>{validationResult.valid ? 'Contexto válido' : 'Contexto incompleto'}</strong>
              ...
              <section className="document-template-generation">
                ...
                <p>El archivo generado quedará guardado en el historial documental.</p>
                <form onSubmit={(event) => void handleGenerationSubmit(event)}>
                  <button className="button button-primary" type="submit" disabled={isGenerating}>
                    {isGenerating ? 'Generando...' : `Generar ${FORMAT_LABELS[outputFormat]}`}
                  </button>
                </form>
                ...
                {generationResult ? (
                  <div className="document-template-generation-result">
                    ...
                    <p>Tamaño: {generationResult.fileSizeBytes} bytes · ID: {generationResult.documentId}</p>
                    <a className="button button-primary document-template-download-link" href={generationResult.downloadUrl}>
                      Descargar documento
                    </a>
                  </div>
                ) : null}
            ...
            <p className="document-templates-empty">Selecciona una plantilla para ver el detalle.</p>
          )}
        </article>
      </section>
    </main>
