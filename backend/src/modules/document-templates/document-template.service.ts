...
} from './document-template.registry.js';
...
export function listDocumentTemplates(
  ...
  return DOCUMENT_TEMPLATE_REGISTRY
    .filter((template) => {
      ...
      if (
        filters.scope
        && !template.scope.includes(filters.scope)
      ) {
        return false;
      }
      ...
      if (
        ...
        && !template.outputFormats.includes(
          filters.outputFormat,
        )
      ) {
        return false;
...
export function getDocumentTemplateByCode(
  code: string,
): DocumentTemplateDefinition | null {
  const normalizedCode = code.trim();

  return DOCUMENT_TEMPLATE_REGISTRY.find(
    (template) =>
...
export function getDocumentTemplateRequiredInputs(
  code: string,
): DocumentTemplateInputDefinition[] | null {
  const template =
    getDocumentTemplateByCode(code);
...
export function validateDocumentTemplateContext(
  code: string,
  ...
  const unsupportedFormat =
    ...
    && !template.outputFormats.includes(
      requestedFormat as DocumentOutputFormat,
    );

  const missingInputs =
    template.requiredInputs.filter((input) => {
      if (!input.required) {
        return false;
      }
  ...
  const issues = missingInputs.map(
    (input) => ({
      field: input.key,
      message:
        `El campo "${input.label}" es obligatorio para la plantilla "${template.name}".`,
    }),
  );
  ...
  if (
    ...
    issues.push({
      field: 'outputFormat',
      message:
        `El formato "${requestedFormat}" no está soportado por la plantilla "${template.name}".`,
    });
  }
